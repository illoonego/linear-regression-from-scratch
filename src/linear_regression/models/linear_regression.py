...
class LinearRegression:
    ...
    def fit(self, X, y, method='gradient_descent'):
        ...
        if y.shape[0] != n_samples:
            raise ValueError(f"Number of samples in X and y must be equal."
                             f"X: {n_samples}, y: {y.shape[0]}")
        ...
        if n_samples < n_features + 1:
            warnings.warn(f"Number of samples ({n_samples}) should be greater than "
                  f"number of features ({n_features}) for reliable results")
